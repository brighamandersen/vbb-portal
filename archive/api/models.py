...
class MentorProfile(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.SET_NULL, related_name="mp", null=True, blank=True
    )
...
class MenteeComputer(models.Model):
    ...
    language = models.ForeignKey(
        Language, on_delete=models.SET_NULL, related_name="computers", null=True,
    )
...
class SessionSlot(models.Model):
    ...
    msm = models.PositiveIntegerField(
        null=True, validators=[MinValueValidator(0), MaxValueValidator(10079)]
    )
    ...
    def __str__(self):
        return aux_fns.display_day("US/Eastern", self.msm, self.end_date,)

    def display(self):
        return aux_fns.display_day(
            self.mentor.mp.time_zone, self.msm, self.end_date, True
        )
